              ...
              return (
                ...
                >
                  ...
                  >
                     ...
                     <div className={`w-[44px] h-[44px] md:w-[46px] md:h-[46px] rounded-full flex items-center justify-center transition-all motion-reduce:transition-none duration-500 transform ${isActive ? 'bg-black text-white scale-100 rotate-0' : 'bg-black/85 text-white/80 scale-0 -rotate-45 group-hover:scale-100 group-hover:text-white'}`}>
        ...
        <div className="lg:hidden flex flex-col gap-5">
          {services.map((service, index) => {
            ...
            return (
              <div
                key={service.id}
                className={`overflow-hidden rounded-[20px] transition-all motion-reduce:transition-none duration-500 ${isActive ? 'bg-white shadow-[0_15px_40px_-10px_rgba(0,0,0,0.1)] border border-gray-100' : 'bg-white/60 shadow-sm border border-gray-100/50'}`}
              >
                <div
                  className={`cursor-pointer p-6 flex justify-between items-center transition-all motion-reduce:transition-none duration-300 active:scale-[0.98] ${isActive ? 'bg-white' : 'bg-transparent'}`}
                ...
                >
                  <div className="flex items-center gap-5">
                    ...
                    <h4 className={`text-[20px] font-display font-medium tracking-tight transition-colors duration-300 ${isActive ? 'text-primary-dark' : 'text-gray-600'}`}>
                      {service.title}
                    </h4>
                  </div>
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all motion-reduce:transition-none duration-500 flex-shrink-0 ${isActive ? 'bg-primary-dark text-white rotate-45 shadow-md' : 'bg-gray-100 text-gray-400'}`}>
                    ...
                    >
                      <div className="p-6 pt-2">
                        <motion.div 
                          ...
                          className="relative h-[260px] rounded-[16px] overflow-hidden mb-6 shadow-md"
                        >
                          <img
                            src={service.img}
                            alt={service.title}
                          ...
                          <div className="absolute bottom-4 left-4 right-4">
                             <span className="bg-white/95 backdrop-blur-md px-4 py-1.5 rounded-full text-[11px] font-bold uppercase tracking-widest text-primary-dark shadow-sm">
                               {service.tag}
                             </span>
                          </div>
                        </motion.div>
                        <p className="text-[16px] text-gray-600 leading-[1.8] font-normal">
                          {service.description}
                        </p>
                      </div>
                    </motion.div>
          ...
          <div className="mt-8 flex justify-center w-full">
            <Link
              to="/services"
              className="w-full sm:w-auto inline-flex items-center justify-center gap-3 rounded-full border border-primary-dark/10 bg-white px-8 py-5 text-[13px] font-bold uppercase tracking-[2px] text-primary-dark shadow-md active:scale-95 transition-transform"
            >
